...
export default function PeopleDetails() {
    ...
    return (
        <div className="wd-people-details position-fixed top-0 end-0 bottom-0 bg-white p-4 shadow w-25">
            ...
            {!editing && (
                <div><b>Email:</b>           <span className="wd-email">         {user.email}         </span> <br /></div>
            )}
            {user && editing && (
                <input className="form-control w-50 wd-edit-email" type="email"
                    defaultValue={`${user.email}`}
                    onChange={(e) => setEmail(e.target.value)}
                    onKeyDown={(e) => {
                        if (e.key === "Enter") { saveUser(); }
                    }} />)}
            {!editing && <div><b>Roles:</b>           <span className="wd-roles">         {user.role}         </span> <br /></div>}
            {user && editing && <div>
